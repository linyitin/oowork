...
while True:
	...
	start=time.time()
	try:
		s.sendto(packet,(hinet_dns_server,53))
		s.recvfrom(1024)
		ArrivalTime=time.strftime("%Y-%m-%d_%H:%M:%S",time.localtime())
		end=time.time()-start
		ans={"TransmissionTime":TransmissionTime,"ArrivalTime":ArrivalTime,"DnsIp":hinet_dns_server,"time":end}
		json_str=json.dumps(ans)
		sk.sendto(json_str,splunk_host)
		time.sleep(10)
	except:
		ans={"TransmissionTime":TransmissionTime,"ArrivalTime":ArrivalTime,"DnsIp":hinet_dns_server,"error":"timeout"}
		json_str=json.dumps(ans)
		sk.sendto(json_str,splunk_host)
		time.sleep(10)
